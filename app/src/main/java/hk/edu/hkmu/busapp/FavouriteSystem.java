...
public class FavouriteSystem {
    ...
    public static class FavouriteSystemItem{
        ...
        public FavouriteSystemItem(String route, String type, String bound, String stopId) {
            this.route = route;
            this.type = type;
            this.bound = bound;
            this.stopId = stopId;
    ...
    public void setFavouriteRoute(FavouriteSystemItem favouriteSystemItem){
        ...
        Toast.makeText(context, "已加入收藏", Toast.LENGTH_SHORT);
        prefsEditor.commit();
    }

    public List<FavouriteSystemItem> getFavouriteRoute(){
        Map<String, ?> allFavourite = sharedPref.getAll();
        List<FavouriteSystemItem> list = new ArrayList<>();
        for (Map.Entry<String, ?> entry : allFavourite.entrySet()) {
